...
// EnsureKindConfig writes a minimal kind config file when none is
// configured. Dies when cfg.KindConfig is set but the file is
// missing. Otherwise creates an ephemeral kind-config under
// $TMPDIR, registers it for cleanup, and stores the path on
// cfg.KindConfig.
