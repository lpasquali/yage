...
# K3s mode: the Go binary renders an embedded K3s flavor manifest
# (internal/capimanifest/k3s_template.yaml) directly; no clusterctl
# generate cluster --flavor k3s and no CAPMOX fork required.
...
# Cilium on management: Hubble on (cheap observability), LB-IPAM off (no
# Service type=LoadBalancer on a stateless single-node mgmt).
MGMT_CILIUM_HUBBLE="${MGMT_CILIUM_HUBBLE:-true}"
MGMT_CILIUM_LB_IPAM="${MGMT_CILIUM_LB_IPAM:-false}"
# Proxmox CSI on management: off by default. Management runs CAPI
# controllers + bootstrap state only — no PVCs. Set true to opt in.
MGMT_PROXMOX_CSI_ENABLED="${MGMT_PROXMOX_CSI_ENABLED:-false}"
